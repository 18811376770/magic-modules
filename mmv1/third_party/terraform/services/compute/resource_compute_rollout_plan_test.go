		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceName, "waves.0.orchestration_options.0.delays.0.duration.0.seconds", "60"),
  ...
  waves {
    ...
    selectors {
      resource_hierarchy_selector {
        included_projects = ["projects/${data.google_client_config.current.project}"]
      }
      location_selector {
        included_locations = ["us-central1-a"]
      }
